...
def find_top(input_array):
    for i in range(len(input_array[0])):
        for j in range(len(input_array)):
            if input_array[j][i]==255:
                return j
...
input_image = "C:/Users/minby/Desktop/pictures/pas/p10_10/O1.jpg"

image = cv2.imread(input_image, cv2.IMREAD_GRAYSCALE)
if image is None: raise Exception("이미지파일 읽기 오류")
image = cv2.resize(image, (321, 600))

image_c = cv2.imread(input_image, cv2.IMREAD_COLOR)
...
print("top:",find_top(canny))
canny = cv2.cvtColor(canny,cv2.COLOR_GRAY2RGB)
#canny = remove_grabcut_bg(canny)
